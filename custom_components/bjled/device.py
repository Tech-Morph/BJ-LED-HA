"""Shared BLE connection manager for a BJ_LED device.
...
class BjLedDevice:
    """Owns the single BLE connection to one BJ_LED device."""
    ...
    def _on_unexpected_disconnect(self, client: BleakClientWithServiceCache) -> None:
        _LOGGER.warning("BJ_LED %s disconnected unexpectedly", self.address)
        self._client = None
        ...
        async with self._lock:
            try:
                ...
                if not retry:
                    raise
                _LOGGER.debug(
                    "BJ_LED %s write failed (%s), reconnecting once",
                    self.address,
                    err,
                )
                client = await self._ensure_connected_locked()
    ...
    async def _keep_alive_tick(self, _now) -> None:
        ...
        async with self._lock:
            try:
                ...
                self._client = None
                _LOGGER.debug(
                    "BJ_LED %s keep-alive failed, will retry next tick", self.address
                )
